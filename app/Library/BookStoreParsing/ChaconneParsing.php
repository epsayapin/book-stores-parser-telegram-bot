...
class ChaconneParsing implements BookStoreParsingInterface{
	...
	public static function getSearchResultPage(
		...
		if($booksCrawlerCount > 0){
			...
			for($i = 0; $i < $booksCrawlerCount; $i++)
			{
				if($booksCrawler->eq($i))
					{
						$book = [];

						$book["title"] = $booksCrawler->eq($i)->filter('.title')->text();
						$link = $booksCrawler->eq($i)->filter(".title")->attr('href');
	...
	{
		$url = self::BOOKCARD_URL . $bookCode . '/';
